  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (fromFile.ok) {
        ...
        setFileReady(false);
        setSaveError((fromFile as any).error);
      }
      pastRef.current = [];
      futureRef.current = [];
    ...
    timer.current = setTimeout(() => {
      ...
      void saveToFile(state).then((fileResult) => {
        if (fileResult.ok && localResult.ok) {
          ...
          setSaveError((fileResult as any).error);
        } else if (!fileResult.ok) {
          // Local cache succeeded but file save failed — work isn't git-portable yet.
          setSavedAt(Date.now());
          setSaveError(`File save failed: ${(fileResult as any).error}`);
        } else {
          setSavedAt(Date.now());
          setSaveError(localResult.ok ? null : (localResult as any).error);
        }
      });
    }, SAVE_DEBOUNCE_MS);
    return () => {
