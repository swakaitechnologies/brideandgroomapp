...
function parseTextWithAsterisks(text: string, theme: Theme) {
  if (!text) return "";
    ...
    if (part.startsWith("*") && part.endsWith("*")) {
      ...
      return (
        <span
          key={index}
          style={{
            fontFamily: "'Newsreader', 'Playfair Display', 'Source Serif Pro', Georgia, serif",
            fontStyle: "italic",
            fontWeight: 400,
            fontSize: "1.05em",
...
function EditableText({
  ...
  if (!editable) {
    return (
      <div style={{ whiteSpace: multiline ? "pre-wrap" : "nowrap", ...style }}>
        {theme ? parseTextWithAsterisks(value, theme) : value}
      </div>
    );
  }
      ...
      ref={(el) => {
        if (el && document.activeElement !== el) {
          ...
          try {
            ...
            console.warn("Failed to set cursor selection:", e);
          }
        }
      }}
      contentEditable
...
function backgroundFor(theme: Theme, inverted?: boolean) {
  if (theme.id === "dreamy-pastel") {
    if (inverted) {
      return "linear-gradient(180deg, #FBE3D7 0%, #F7C9D6 45%, #F3B6CC 100%)";
    }
    return "linear-gradient(180deg, #DDEBFA 0%, #F5E0F0 45%, #FCEFD6 100%)";
  }
  if (inverted) {
...
export function SlideCanvas({
  ...
  return (
    ...
    >
      <SlideBackground slide={slide} cW={cW} theme={theme} />
      <SlideElements
        slide={slide}
        device={device}
        orientation={orientation}
