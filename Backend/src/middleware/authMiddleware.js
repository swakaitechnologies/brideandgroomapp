...
const protect = async (req, res, next) => {
  ...
  try {
    ...
    const user = await User.findByPk(decodedToken.userId, {
      attributes: ["id", "isBlocked", "isDeleted", "isOnline", "lastSeen"],
    });
    ...
    // Dynamic heartbeat: set isOnline = true and update lastSeen if older than 2 minutes
    try {
      const now = new Date();
      if (!user.isOnline || !user.lastSeen || (now - new Date(user.lastSeen)) > 2 * 60 * 1000) {
        user.isOnline = true;
        user.lastSeen = now;
        await user.save();
      }
    } catch (dbErr) {
      console.error("Auth Middleware Online Status Update Error:", dbErr);
    }
