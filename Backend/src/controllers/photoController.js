...
exports.uploadPhotos = async (req, res) => {
  try {
    ...
    for (const file of req.files) {
      ...
      // 5. Save to DB
      const hasMain = await Photo.findOne({ where: { userId, isMain: true } });
...
exports.deletePhoto = async (req, res) => {
  try {
    ...
    if (wasMain) {
      ...
      if (remainingPhoto) {
        await remainingPhoto.update({ isMain: true });
      }
    }
...
exports.setPrimaryPhoto = async (req, res) => {
  try {
    ...
    console.log(`Setting all photos to non-main for user: ${userId}`);
    await Photo.update({ isMain: false }, { where: { userId } });

    // 3. Set this photo to isMain: true
    console.log(`Setting photo ${id} as main`);
